...
* Time complexity: $$O(nlog(n))$$.
    ...
    Time taken to sort the array is $$O(nlog(n))$$ (average case). Time taken for linear iteration through the array is of $$O(n)$$ complexity. Hence overall time complexity is $$O(nlog(n))$$.
