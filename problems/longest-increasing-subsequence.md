...
do this, we make use of a recursive function $$lengthofLIS$$ which returns the length of the LIS possible from the current element(corresponding to $$curpos$$)
...
a particular call in a 2-d memorization array $$memo$$. $$memo[i][j]$$ represents the length of the LIS possible using $$nums[i]$$ as the previous element considered to
